...
class PmergeMe
{
  private:
    ...
    template <typename StateType> void fillPairs(StateType &state)
    {
        state.pairs.clear();
        if (state.A.size() % 2 != 0)
        {
    ...
    template <typename StateType> void generateJacobsthal(StateType &state)
    {
        state.jacob.clear();
    ...
    template <typename StateType> void pendToMain(StateType &state)
    {
        ...
        {
            typename StateType::A_type::iterator it =
                std::lower_bound(state.mainChain.begin(), state.mainChain.end(), state.straggler);
  ...
  public:
    ...
    template <typename Container> void parser(std::vector<std::string> args, Container &target)
    {
        char *endptr;
    ...
    template <template <typename, typename> class ContainerTemplate> void sort(std::vector<std::string> args)
    {
    ...
    template <typename F> double measureTime(F sortFunction, std::vector<std::string> args)
    {
